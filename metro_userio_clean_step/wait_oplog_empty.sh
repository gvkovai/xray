...
#
# Purpose:  Wait for the Oplog to be empty for a pre-determined number of
#           seconds before returning.
#

if [ "$#" -lt 1 ]; then
  echo "Please provide indicate the number of seconds the Oplog should be empty before returning."
...
while [ $EMPTY_SECS -lt $1 ] && [ $NUM_CHK -lt $MAX_CHK ] ; do
  ...
  echo "OPLOG_SIZE="${OPLOG_SIZE}  "EMPTY_SECS="$EMPTY_SECS "NUM_CHK="$NUM_CHK
  if [ "${OPLOG_SIZE}" == "0.0" ] ; then
    ((EMPTY_SECS += 1))
  else
    EMPTY_SECS=0
  fi
